...
class BaseVisualizer:
    ...
    def _setup_grid(self, init_entries: list[CellLogEntry]) -> None:
        ...
        self.all_polygons = []
        self.cell_coords = []  # (x, y) for each cell for easy lookup later
        for entry in init_entries:
            polygon = mpatches.RegularPolygon((entry.x, entry.y),
                                              numVertices=6, radius=self.cell_size, orientation=0)
            self.all_polygons.append(polygon)
            self.cell_coords.append((entry.x, entry.y))

        # Assign initial facecolors based on cell state
        # colors = [self._get_cell_color(entry) for entry in init_entries]
        ...
        self.all_cells_coll = PatchCollection(self.all_polygons, facecolors=self.cell_colors, match_original=True, zorder=3)
