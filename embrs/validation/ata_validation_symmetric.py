"""
Symmetric ATA (Arrival Time Agreement) validation.

This is a drop-in companion to ata_validation.py. The only meaningful
difference is in compute_ATA_from_points: the both-burned discrepancy
contribution uses the absolute difference |t_sim - t_ref|, matching the
standard formulation in the fire-model validation literature, instead of
the asymmetric max(t_sim - t_ref, 0) used in the original script.

All other behaviour (data loading, evaluation grid, plotting) is identical
to ata_validation.py so that ATA values produced by the two scripts can be
directly compared for the same case.
"""
...
def compute_ATA_from_points(eval_pts, sim_tree, hexes, sim_times,
    ...
    Discrepancy contributions:
        - Both burned:        |t_sim - t_ref|        (symmetric — penalizes
                                                       both early and late
                                                       arrival)
        - Only sim burned:    max_time - t_sim       (overprediction in extent)
        - Only ref burned:    max_time - t_ref       (underprediction in extent)

    Cells unburned in both models are excluded from the union.
    """
...
print(f"Arrival Time Agreement (symmetric ATA): {ata_score:.3f}")
