...
def test_default_bi_filter_mode_is_mean_daily_peak():
    cfg = Config(**_minimal_kwargs())
    assert cfg.bi_filter_mode == "mean_daily_peak"


def test_bi_filter_mode_accepts_valid_modes():
    for mode in ("mean_daily_peak", "mean_1pm", "peak_only"):
...
def test_columns_for_bi_filter_mode():
    ...
    assert columns_for_bi_filter_mode("mean_daily_peak") == ("mean_daily_peak_bi",)
    assert columns_for_bi_filter_mode("mean_1pm") == ("mean_daily_1pm_bi",)
...
def test_score_distance_column_for_mode():
    ...
    assert score_distance_column_for_mode("mean_daily_peak") == "mean_daily_peak_bi"
    assert score_distance_column_for_mode("mean_1pm") == "mean_daily_1pm_bi"
