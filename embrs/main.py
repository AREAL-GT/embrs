...
from embrs.utilities.file_io import read_fms_file
...
class ConsoleLoader:
    """Headless stand-in for ``LoaderWindow``.

    Mirrors the ``LoaderWindow`` interface but prints status to stdout
    instead of opening a Tk window, so simulations run on headless nodes
    (e.g. PACE compute nodes) where no display is available.
    """
...
def initialize(sim_params: SimParams) -> Tuple[FireSim, bool, "RealTimeVisualizer"]:
    ...
    if sim_params.visualize:
        # Imported lazily: the visualizer pulls in matplotlib/Tk, which are
        # unavailable (and unneeded) on headless nodes.
...
def sim_loop(sim_params: SimParams):
    ...
    # Create loader window object. Use a console-only loader on headless
    # nodes (no display), otherwise the Tk progress window.
    if is_headless():
        loader_window = ConsoleLoader('Fire sim', max_value=num_runs*(duration/time_step)+1)
    else:
        from embrs.utilities.file_io import LoaderWindow
...
def load_sim_params(cfg_path: str) -> SimParams:
    ...
    solar_source = config["Weather"].get("solar_source", default_solar_source()).strip().lower()
    if solar_source not in ("openmeteo", "offline"):
        raise ValueError(
            f"Error in {cfg_path}: 'solar_source' must be 'openmeteo' or 'offline', got {solar_source!r}."
        )
...
def main():
    ...
    if args.config:
        ...
        from embrs.utilities.file_io import SimFolderSelector
